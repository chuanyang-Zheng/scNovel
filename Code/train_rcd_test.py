...
from test_rcd import test_performance,test_performance_knn,test_performance_lof
...
def eval_performance(net,epoch):
    ...
    reference_this="all"
    # args.sample_ratio=1
    args.method_name=args.loss+"_"+args.score+"_"+str(args.react)+str(args.sample_ratio)+"_"+reference_this
    test_performance_knn(net,args,test_loader,test_ood_loader,args.T,args.noise,args.score,1,os.path.join(save_path, str(args.method_name)+'_testing_results.csv'),epoch,train_loader=train_loader,num_classes=num_classes,save_logit= os.path.join(save_path, path +
                                    '_epoch_' + str(epoch) + '.npz'),reference=reference_this,test_testOOD=test_testood_loader)
    ...
    # test_performance_knn(net, args, test_loader, test_ood_loader, args.T, args.noise, args.score, 1,
    #                      os.path.join(save_path, str(args.method_name) + '_testing_results.csv'), epoch,
    #                      train_loader=train_loader, num_classes=num_classes)
    #
    # args.score = "knn_ALL"
    # args.T = 1000
    ...
    # args.T = 1000
    # args.method_name = args.loss + "_" + args.score + "_" + str(args.react)
    # test_performance_lof(net, args, test_loader, test_ood_loader, args.T, args.noise, args.score, 1,
    #                      os.path.join(save_path, str(args.method_name) + '_testing_results.csv'), epoch,
    #                      train_loader=train_loader, num_classes=num_classes)
    ...
    #                               '_testing_results.csv'), epoch, train_loader)
    # args.score = "lof"
    # args.T = 1000
    # args.method_name = args.loss + "_" + args.score + "_" + str(args.react)
    # test_performance(net, args, test_loader, test_ood_loader, args.T, args.noise, args.score, 1,
